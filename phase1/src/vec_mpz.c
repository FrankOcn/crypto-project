...
struct vec_mpz_t* vec_mpz_product_tree(struct vec_mpz_t* vec, const unsigned int BATCH_SIZE){
  int i;
  struct vec_mpz_t* product_tree = vec_mpz_init_size((BATCH_SIZE * 2) - 1);
  for (i = 0; i < BATCH_SIZE; ++i) {
    mpz_set(product_tree->elems[i + BATCH_SIZE - 1], vec->elems[i]);
  }
  for (i = product_tree->size - BATCH_SIZE - 1; i >= 0; --i) {
    mpz_mul(
...
{
  struct vec_mpz_t* squares = vec_mpz_init_size(BATCH_SIZE);
  ...
  for (int i = 0; i < BATCH_SIZE; i++)
  {
    ...
    if (mpz_cmp(temp, batch->elems[i]) == 0)
    {
      smooth_bool_arr[i] = 1;
    }
    else
...
/*****
 * Remove gcd checking, we only care if the number is fully smooth
 * ***
...
{
  struct vec_mpz_t* batch_product_tree;
  ...
  batch_product_tree = vec_mpz_product_tree(batch, BATCH_SIZE);
  batch_remainder_tree = vec_mpz_remainder_tree(z, batch_product_tree, BATCH_SIZE);
  batch_remainders = remainders_in(batch_remainder_tree, BATCH_SIZE);
  compute_smooth(batch_remainders, batch, smooth_bools, BATCH_SIZE);


  mpz_clear(p);
  mpz_clear(remainder);
  vec_mpz_free(batch_product_tree);
